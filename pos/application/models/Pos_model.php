	...
	public function get_details(){
		...
		extract($_POST);
		  ...
		  if(empty($warehouse_id)){
		  	$warehouse_id=$this->db->query("select id from db_warehouse where status=1 order by id")->row()->id;
		  }
		  $table='';
	      $q2=$this->db->query("select b.id as tax_id, a.*,b.tax,b.tax_name,a.tax_type,a.item_image,
	                            (select sum(qty) from db_stockentry where db_stockentry.item_id=a.id and db_stockentry.warehouse_id=$warehouse_id) as stocki
	                            from db_items a,db_tax b where b.id=a.tax_id and a.status=1 $str order by a.stock desc");              
	      if($q2->num_rows()>0){
	        foreach($q2->result() as $res2){
	          ...
	          <div class="box box-default item_box" id="div_'.$res2->id.'" onclick="'.$str.'"
	          				...
	          				data-item-available-qty="'.$res2->stocki.'"
	           	...
	           	<span class="label label-danger push-right" style="font-weight: bold;font-family: sans-serif;" data-toggle="tooltip" title="'.$res2->stocki.' Quantity in Stock">Qty: '.$res2->stocki.'</span>
