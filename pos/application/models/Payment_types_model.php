...
class Payment_types_model extends CI_Model {
	...
	var $column_order = array('payment_type','bank_cash','status'); //set column field database for datatable orderable
	var $column_search = array('payment_type','bank_cash','status'); //set column field database for datatable searchable 
	...
	public function verify_and_save(){
		...
		if($query->num_rows()>0){
			...
			$query1="insert into db_paymenttypes(payment_type,bank_cash,status) values('$payment_type_name','$bank_cash',1)";
	...
	public function get_details($id,$data){
		...
		if($query->num_rows()==0){
			...
			$data['bank_cash']=$query->bank_cash;
			return $data;
		}
	}
	public function update_payment_type(){
		...
		if($query->num_rows()>0){
			...
			$query1="update db_paymenttypes set payment_type='$payment_type_name', bank_cash='$bank_cash' where id=$q_id";
			if ($this->db->simple_query($query1)){
