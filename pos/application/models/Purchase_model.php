...
class Purchase_model extends CI_Model {
	...
	var $column_order = array( 'a.id','a.purchase_date','a.purchase_code','a.reference_no','a.grand_total','a.payment_status','a.created_by','b.supplier_name','c.warehouse_name','a.paid_amount','a.purchase_status','a.return_bit'); //set column field database for datatable orderable
	var $column_search = array('a.id','a.purchase_date','a.purchase_code','a.reference_no','a.grand_total','a.payment_status','a.created_by','b.supplier_name','c.warehouse_name','a.paid_amount','a.purchase_status','a.return_bit'); //set column field database for datatable searchable
	...
	private function _get_datatables_query($supplierId=false)
	{
		...
		if($supplierId){$this->db->where('a.supplier_id',$supplierId);}
	...
	function get_datatables($supplierId=false)
	{
		$this->_get_datatables_query($supplierId);
		if($_POST['length'] != -1)
	...
	public function verify_save_and_update(){
	    ...
	    if($command=='save'){//Create purchase code unique if first time entry
			...
			$qdeletestock=$this->db->query("delete from db_stockentry where purchase_id='$purchase_id'");
			if(!$q11){
				return "failed";
			}
		}
	...
	public function delete_purchase($ids){
		...
		$qdeletestock=$this->db->query("delete from db_stockentry where purchase_id in($ids)");
