...
class Purchase extends MY_Controller {
	...
	public function index()
	{
		...
		if(isset($_GET['supplierId'])){
			...
			$data['page_title']="All Purchases Of ".$_GET['supplierName'];
		}else{
			$supplierId = false;
	...
	{
		$list = $this->purchase->get_datatables(@$_GET['supplierId']);

		$data = array();
		$no = $_POST['start'];
		$no++;
		if($_GET['supplierId']){
			$sup_openint=$this->db->query("select opening_balance from db_suppliers where id=".$_GET['supplierId']."")->row()->opening_balance;
			$sup_openint=$sup_openint>0?$sup_openint:0;
			...
			$row[] = '';
			$row[] = 'Opening Balance';
			$row[] = app_number_format($sup_openint);
            $row[] = '';
            $row[] = '';
            $row[] = '';
