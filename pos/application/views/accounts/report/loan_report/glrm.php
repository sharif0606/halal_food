...
?>
		<div id="display">
			...
			<p class="text-center vHide">Loan Report</p>
			<table id="" class="table table-striped table-bordered table-hover" width="100%">
				<thead>
					<tr>
						<th rowspan="2">Program name</th>
						<th colspan="2">Member at the end of last month</th>
						<th colspan="2">Loan disbursement</th>
						<th rowspan="2">Loan collection this month</th>
						<th rowspan="2">Paid people</th>
						<th colspan="2"> at the end of month</th>
					</tr>
					<tr>
						<td>Loan</td>
				...
				<tbody>
					<tr>
						...
						<td>
						    <?php 
							$countycmpaid=$this->db->query("SELECT count(id) as tm FROM `tbl_loan_assign` WHERE `loan_closing_date`>'$lm' and `loan_closing_date`<='$cm' and status=0 and `member_Id` in (select member_Id from tbl_members WHERE tbl_members.gender=2)")->row();
							$tpaidLoanF=$countycmpaid->tm;
							echo $tpaidLoanF;
							?>
						</td>
						<td>
					...
					<tr>
						...
						<td>
						 <?php 
							$countycmpaid=$this->db->query("SELECT count(id) as tm FROM `tbl_loan_assign` WHERE `loan_closing_date`>'$lm' and `loan_closing_date`<='$cm' and status=0 and `member_Id` in (select member_Id from tbl_members WHERE tbl_members.gender=1)")->row();
							$tpaidLoanM=$countycmpaid->tm;
							echo $tpaidLoanM;
							?>
						</td>
						<td>
					...
					<tr>
						...
						<td><?= $tpaidLoanM+$tpaidLoanF;?></td>
						<td><?= $endOfMonthLoanMemberF+$endOfMonthLoanMemberM - ($tpaidLoanM+$tpaidLoanF)?></td>
