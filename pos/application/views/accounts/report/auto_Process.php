...
<div id="display">
<img src="<?= base_url().'loading/ajax-loader.gif'; ?>" alt="loading" id="loading-image" class="loader">
				...
				<tr>
					...
					<td>
					     <select id="proType" name="proType">
					        ...
					        <option value='2'>Long Term Loan and Deposit</option>
					    </select>
					</td>
					<td>Product Name</td>
					<td>All</td>
					<td>Collection Date</td>
					<td><input class="form-control" id="disbursement_Date" name="payment_Date"  type="text" value="<?= date('Y-m-d', strtotime( $_GET['payment_Date'])); ?>"></td>
					
				</tr>
				<tr>
				   <td colspan="8" class="text-center"><b>Payment Date:</b></b><b><?= date('Y-m-d',strtotime($_GET['date'])); ?></h4></b>
				</tt>
			</table>
...
$('document').ready(function(){
	...
	function view_loan_Lists(){
		...
		var date ='<?php echo $_GET['payment_Date'];?>';
		 ...
		 $("#loading-image").hide();	 
			...
			if(response.length>0){
				...
				for (var i = 0; i < response.length; i++) {
					...
					var tl_Repay 		    = response[i].tl_Repay;
					var udpsAmt 		    = response[i].udpsAmt;
					var first_repay_Date    = response[i].first_repay_Date;
					//alert(rp_status);
