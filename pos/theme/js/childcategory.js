...
$('#save,#update').click(function (e) {
	...
	check_field("childcategory_name");	
    ...
    if(this_id=="save")  //Save start
    {
		if(confirm("Do You Wants to Save Record ?")){
			e.preventDefault();
			data = new FormData($('#childcategory-form')[0]);//form name
			/*Check XSS Code*/
			...
			$.ajax({
				type: 'POST',
				url: base_url+'childcategory/newcategory',
				data: data,
				cache: false,
				contentType: false,
				processData: false,
				success: function(result){
					if(result=="success"){
						window.location=base_url+"childcategory/view";
						return;
	...
	else if(this_id=="update")  //Save start
    {
		if(confirm("Do You Wants to Update Record ?")){
			...
			$.ajax({
				type: 'POST',
				url: base_url+'childcategory/update_category',
				data: data,
				cache: false,
				contentType: false,
				processData: false,
