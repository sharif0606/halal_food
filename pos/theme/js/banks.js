...
$('#save,#update').click(function (e) {
    ...
    if(this_id=="save")  //Save start
    {
					if(confirm("Do You Wants to Save Record ?")){
						...
						e.preventDefault();
						data = new FormData($('#banks-form')[0]);//form name
						/*Check XSS Code*/
             ...
             // alert(result);return;
							if(result=="success")
							{
								...
								window.location=base_url+"banks";
								return;
							}
				...
				if(confirm("Do You Wants to Update Record ?")){
					...
					$.ajax({
					type: 'POST',
					url: base_url+'banks/update_bank',
					data: data,
					cache: false,
					contentType: false,
					processData: false,
...
{
	var base_url=$("#base_url").val().trim();
	$.post(base_url+"banks/update_status",{id:id,status:status},function(result){
		if(result=="success")
				{
...
//Delete Record start
function delete_bank(q_id)
{
	var base_url=$("#base_url").val().trim();
   if(confirm("Do You Wants to Delete Record ?")){
   	$(".box").append('<div class="overlay"><i class="fa fa-refresh fa-spin"></i></div>');
   $.post(base_url+"banks/delete_banks",{q_id:q_id},function(result){
   //alert(result);return;
	   if(result=="success")
				{
